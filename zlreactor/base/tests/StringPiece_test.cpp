...
#define ASSERT_GE(a, b)     CHECK(a >= b)
#define ASSERT_NE(a,b)    CHECK(a != b)
...
// see pcre_stringpiece_unittest.cc
// see https://github.com/chen3feng/toft/blob/master/base/string/string_piece_test.cpp
int main()
{
    CheckSTLComparator();
    ...
    static const char kHello[] = "hellohellohellohellohellohellohello";
    ASSERT_TRUE(StringPiece(kHello) == std::string(kHello));
    
    StringPiece sp("hello");
    ASSERT_EQ(0, sp.ignore_case_compare("HELLO"));
    ASSERT_LT(sp.ignore_case_compare("iELLO"), 0);
    ASSERT_GT(sp.ignore_case_compare("gELLO"), 0);
    ASSERT_TRUE(sp.ignore_case_equal("HELLO"));
